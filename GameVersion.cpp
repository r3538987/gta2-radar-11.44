...
unsigned int _NOINLINE_ detect_game_id() {
    ...
    // Experimental compatibility fallback: plugin-sdk currently has address
    // information only for GTA2 9.6. Treat other GTA2 executables (including
    // 11.44) as 9.6 so version-gated events are installed and plugins can be
    // tested against the existing addresses.
    //
    // This does not make the hard-coded 9.6 addresses safe for other builds.
    return GAME_9600EN;
#endif
#ifdef GTAIV
        return GAME_CE;
